...
async function generateGeminiDoc() {
  try {
    ...
    const mdcFiles = files.filter(file => file.endsWith('.mdc') || file.endsWith('.md'));
    ...
    try {
        ...
        combinedContent += mainReadmeContent.substring(mainReadmeContent.indexOf('\n') + 1);

    } catch (e) {
        console.log("Could not find a README.md in .cursor/rules, starting with a generic title.");
        combinedContent = `# Gemini Assistant Guide for V1 Project\n\nThis document provides essential guidelines for the AI assistant to ensure code generation and modifications align with the project's standards.\n\n`;
    }
