...
function getCurrentEnvironment() {
  ...
  if (!envContent) {
    ...
    console.log('💡 Run "bun run supabase:local" or "bun run supabase:remote" to configure');
    return;
  }
  ...
  if (useLocal) {
    ...
    try {
      const dockerOutput = execSync('docker ps --filter "name=supabase" --format "table {{.Names}}\t{{.Status}}"', { encoding: 'utf8' });
    ...
    if (projectIdMatch) {
      ...
      console.log(`📊 Dashboard: https://supabase.com/dashboard/project/${projectIdMatch[1]}`);
    }
...
console.log(`   Docker: ${checkDockerStatus() ? '✅ Available' : '❌ Not available'}`);
console.log(`   Supabase CLI: ${checkSupabaseCLI() ? '✅ Installed' : '❌ Not installed'}`);
