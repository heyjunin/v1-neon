...
export class ExampleAdvancedSeeder extends BaseSeeder {
  ...
  async run(db: NeonHttpDatabase): Promise<void> {
    ...
    const randomNumbers = fakerUtils.array(
      () => fakerUtils.random().number(1, 100),
      20,
    );
    const randomBooleans = fakerUtils.array(
      () => fakerUtils.random().boolean(),
      10,
    );
    const randomUuids = fakerUtils.array(() => fakerUtils.random().uuid(), 5);

    // Generate dates
    const recentDates = fakerUtils.array(
      () => fakerUtils.date().recent(30),
      10,
    );
    ...
    const placeholderImages = fakerUtils.array(
      () => fakerUtils.image().urlPlaceholder(300, 200),
      5,
    );

    // Generate lorem text
    const sentences = fakerUtils.array(() => fakerUtils.lorem().sentence(), 20);
    const paragraphs = fakerUtils.array(
      () => fakerUtils.lorem().paragraph(),
      10,
    );
