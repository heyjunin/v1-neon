...
export async function seedOrganizations() {
  try {
    ...
    for (const org of createdOrganizations) {
      ...
      const numMembers = Math.min(faker.number.int({ min: 1, max: 3 }), availableUsers.length);
    ...
    for (const org of createdOrganizations.slice(0, 3)) { // Only for first 3 organizations
