  ...
  async run(
    ...
    await this.executeInTransaction(db, async () => {
      ...
      if (existingOrganizations.length > 0 && !options?.force) {
        throw new Error(
          "Organizations already exist in the database. Use --force to override.",
        );
      }
      ...
      for (const org of createdOrganizations.slice(0, 5)) {
        // Only for first 5 organizations
      ...
      if (invitesToInsert.length > 0) {
        ...
        logger.info(
          `✅ Created ${invitesToInsert.length} organization invites`,
        );
      }
