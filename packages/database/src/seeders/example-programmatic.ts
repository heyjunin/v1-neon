...
export async function seedWithCustomData(): Promise<SeederResult> {
  ...
  try {
    ...
    return {
      success: true,
      message: "Custom seeding completed successfully",
      data: results,
    };
  } catch (error) {
...
export function generateCustomData(): CustomData {
...
export async function conditionalSeeding(): Promise<SeederResult> {
  const orchestrator = new SeederOrchestrator(db);
  orchestrator.registerMany(seeders);
  ...
  return {
    success: true,
    message: `Seeding completed for environment: ${environment}`,
    data: results,
  };
}

// Example of seeding with custom faker configuration
export async function seedWithCustomFaker(): Promise<SeederResult> {
  ...
  return {
    success: true,
    message: "Custom faker seeding completed",
    data: { techUsers, techPosts },
  };
}
