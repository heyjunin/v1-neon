...
export function getSeederFilePath(seederName: string): string {
  ...
  return path.join(projectRoot, 'packages/database/src/seeders', `${seederName}.ts`);
}

export function getMigrationFilePath(migrationName: string): string {
  const projectRoot = process.cwd();
  return path.join(projectRoot, 'packages/database/src/migrations', `${migrationName}.sql`);
}

export function getSchemaFilePath(tableName: string): string {
  const projectRoot = process.cwd();
  return path.join(projectRoot, 'packages/database/src/schema', `${tableName}.ts`);
}
