...
export async function parseSchemaFile(tableName: string): Promise<SchemaInfo | null> {
...
function parseSchemaContent(content: string, tableName: string): SchemaInfo {
  ...
  while ((match = fieldRegex.exec(content)) !== null) {
    ...
    fields.push({
      ...
      nullable: safeModifiers.includes('.nullable()') || !safeModifiers.includes('.notNull()'),
...
export function generateFakerDataFromSchema(fields: SchemaField[]): string {
  const fakerFields = fields
    .filter(field => !field.isPrimary && field.name !== 'createdAt' && field.name !== 'updatedAt')
    .map(field => {
