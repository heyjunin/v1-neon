...
export function makeMigrationCommand() {
  return new Command("make:migration")
    ...
    .option(
      "--template <template>",
      "Template type (basic|create|alter|drop)",
      "basic",
    )
...
async function createMigration(name: string, options: any) {
  ...
  if (checkFileExists(filePath) && !force) {
    throw new Error(
      `Migration file already exists: ${filePath}. Use --force to overwrite.`,
    );
  }
  ...
  console.log(
    chalk.yellow(`   1. Edit the migration file to add your changes`),
  );
  console.log(
    chalk.yellow(`   2. Run: bun run db:migrate to apply the migration`),
  );
  console.log(
    chalk.yellow(`   3. Or use Drizzle Kit: bun run db:push for development`),
  );
}
