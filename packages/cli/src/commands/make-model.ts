...
import {
  basicModelTemplate,
  fullModelTemplate,
  withRelationsModelTemplate,
} from "../templates/model/index.js";
...
export function makeModelCommand() {
  return new Command("make:model")
    ...
    .option(
      "-t, --template <template>",
      "Template type (basic|with-relations|full)",
      "basic",
    )
...
async function createModel(name: string, options: any) {
  ...
  if (checkFileExists(filePath) && !force) {
    throw new Error(
      `Model file already exists: ${filePath}. Use --force to overwrite.`,
    );
  }
  ...
  if (seed) {
    ...
    try {
      ...
      console.log(
        chalk.yellow(
          "⚠️  Could not create seeder automatically. You can create it manually with:",
        ),
      );
      console.log(
        chalk.yellow(
          `   bun run make:seeder ${tableName} --table=${tableName} --template=faker`,
        ),
      );
    }
  }
  ...
  if (migration) {
    ...
    console.log(
      chalk.yellow(
        "⚠️  Migration creation not implemented yet. You can create it manually with:",
      ),
    );
    console.log(
      chalk.yellow(
        `   bun run make:migration create_${tableName}_table --table=${tableName} --template=create`,
      ),
    );
  }
  ...
  console.log(
    chalk.yellow(`   2. Run: bun run db:generate to generate migrations`),
  );
  console.log(
    chalk.yellow(`   3. Run: bun run db:migrate to apply migrations`),
  );
  if (seed) {
    console.log(
      chalk.yellow(
        `   4. Edit the seeder and run: bun run seed:run ${tableName}-seeder`,
      ),
    );
  }
}
